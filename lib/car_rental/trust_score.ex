defmodule CarRental.TrustScore do
  ...
  def calculate_score(params) do
    with :ok <- clients_limit_not_exceeded?(params),
      ...
      Enum.map(params.clients, fn client ->
        %Response{id: client.client_id, score: :rand.uniform(100)}
      end)
    end
  end

  defp clients_limit_not_exceeded?(%{clients: clients}) when length(clients) <= @clients_limit, do: :ok

  defp clients_limit_not_exceeded?(_),
    do: {:error, "Cannot calculate trust score for more than #{@clients_limit} clients at the same time"}
end
